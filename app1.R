...
library(readr)
library(dplyr)
library(ggplot2)  # Agregamos ggplot2
library(readxl)
...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    tableOutput("tabla_empleo")  # Debes renderizar la tabla aquí
  ),
  navbarMenu(
    title = "Gráficos",
    tabPanel(
      title = "Análisis por región de empleos agricultura",
      plotOutput("grafico_barras")  # Cambiamos la renderización de los gráficos
    ),
    tabPanel(
      title = "Análisis correlación",
      selectInput(inputId = "anyo", label = "Escoja un año", choices = unique(datos_empleo$anyo)),
      actionButton(inputId = "filtrar", label = "Filtrar"),
      plotOutput("grafico_dispersion")  # Cambiamos la renderización de los gráficos
    ),
    tabPanel(
      title = "Analítica Avanzada",
      plotOutput("grafico")  # Aquí deberías definir la renderización del tercer gráfico
    )
  )
)
    ...
    observeEvent(input$filtrar, {
      ...
      if (nrow(datos_filtrados) > 0) {
        grafico_disp <- ggplot(data = datos_filtrados) +
          geom_point(mapping = aes(x = desempleo_mujeres, y = desempleo_hombres), color = "blue") +
          labs(title = "Relación entre mujeres y hombres sin empleo", x = "Mujeres desempleadas", y = "Hombres desempleados") +
          theme_minimal()
